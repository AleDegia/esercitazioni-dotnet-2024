...
class Database : DbContext
{
    ...
    public void AddPurchase(string scelta, User user, Controller controller)
    {
        ...
        using (var context = new Database())
        {
            ...
            if (Int32.Parse(scelta) >= 0 && Int32.Parse(scelta) <= context.Products.Count())
            {

                if (userr.Balance - prod.Price >= 0)
                {
                    var purchase = new Purchase { IdUtente = idUtente, IdProdotto = prod.Id, Prezzo = prod.Price };
            ...
            {
                System.Console.WriteLine("scelta non valida! inserisci l'input giusto");
                controller.RichiestaAcquisto(user);
            }
        }
